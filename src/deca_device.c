...
#include "deca/deca_types.h"
#include "deca/deca_param_types.h"
...
int dwt_initialise(uint16 config)
{
    ...
    pdw1000local->otprev =
            (otp_addr >> 8) & 0xff;            // OTP revision is next byte
    ...
    pdw1000local->sysCFGreg = dwt_read32bitreg(
            SYS_CFG_ID); // Read sysconfig register
...
void dwt_configure(dwt_config_t *config)
{
    ...
    uint8 bw = ((chan == 4) || (chan == 7)) ? 1
                                            : 0; // Select wide or narrow band
    ...
    dwt_write8bitoffsetreg(SYS_CTRL_ID, SYS_CTRL_OFFSET, SYS_CTRL_TXSTRT |
...
int32 dwt_readcarrierintegrator(void)
{
    ...
    for(j = 2;
        j >= 0; j--)  // arrenge the three bytes into an unsigned interger value
    {
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics)
{
    ...
    diagnostics->firstPath = dwt_read16bitoffsetreg(RX_TIME_ID,
    ...
    diagnostics->firstPathAmp1 = dwt_read16bitoffsetreg(RX_TIME_ID,
...
void dwt_readtxtimestamp(uint8 *timestamp)
{
    dwt_readfromdevice(TX_TIME_ID, TX_TIME_TX_STAMP_OFFSET,
            TX_TIME_TX_STAMP_LEN, timestamp); // Read bytes directly into buffer
}
...
{
    return dwt_read32bitoffsetreg(TX_TIME_ID,
            1); // Offset is 1 to get the 4 upper bytes out of 5
}
...
{
    return dwt_read32bitoffsetreg(RX_TIME_ID,
            1); // Offset is 1 to get the 4 upper bytes out of 5
}
...
{
    return dwt_read32bitoffsetreg(SYS_TIME_ID,
            1); // Offset is 1 to get the 4 upper bytes out of 5
}
        ...
        )
{
    ...
    {
        header[cnt++] = 0x80 |
                        recordNumber; // Bit-7 is WRITE operation, bit-6 zero=NO sub-addressing, bits 5-0 is reg file id
    } else {
#ifdef DWT_API_ERROR_CHECK
        assert((index <= 0x7FFF) && ((index + length) <= 0x7FFF)); // Index and sub-addressable area are limited to 15-bits.
#endif
        header[cnt++] = 0xC0 |
                        recordNumber; // Bit-7 is WRITE operation, bit-6 one=sub-address follows, bits 5-0 is reg file id

        if(index <=
           127) // For non-zero index < 127, just a single sub-index byte is required
        {
            header[cnt++] = (uint8) index; // Bit-7 zero means no extension, bits 6-0 is index.
        } else {
            header[cnt++] = 0x80 |
                            (uint8) (index); // Bit-7 one means extended index, bits 6-0 is low seven bits of index.
            header[cnt++] = (uint8) (index
                    >> 7); // 8-bit value = high eight bits of index.
        }
    }
        ...
        )
{
    ...
    if(index == 0) // For index of 0, no sub-index is required
    {
        ...
        assert((index <= 0x7FFF) && ((index + length) <= 0x7FFF)); // Index and sub-addressable area are limited to 15-bits.
#endif
        header[cnt++] = (uint8) (0x40 |
                                 recordNumber); // Bit-7 zero is READ operation, bit-6 one=sub-address follows, bits 5-0 is reg file id
...
void dwt_isr(void)
{
    uint32 status = pdw1000local->cbData
                            .status = dwt_read32bitreg(
    ...
    if(status & SYS_STATUS_RXFCG) {
        ...
        if(finfo16 & RX_FINFO_RNG) {
            pdw1000local->cbData
                    .rx_flags |= DWT_CB_DATA_RX_FLAG_RNG;
        }

        // Report frame control - First bytes of the received frame.
        dwt_readfromdevice(RX_BUFFER_ID, 0, FCTRL_LEN_MAX, pdw1000local->cbData
                .fctrl);
        ...
        // This issue is not documented at the time of writing this code. It should be in next release of DW1000 User Manual (v2.09, from July 2016).
        if((status & SYS_STATUS_AAT) && ((pdw1000local->cbData
                                                  .fctrl[0] &
                                          FCTRL_ACK_REQ_MASK) == 0)) {
            dwt_write32bitreg(SYS_STATUS_ID,
                    SYS_STATUS_AAT); // Clear AAT status bit in register
            pdw1000local->cbData
                    .status &= ~SYS_STATUS_AAT; // Clear AAT status bit in callback data register copy
            pdw1000local->wait4resp = 0;
        }
...
void dwt_forcetrxoff(void)
{
    ...
    dwt_write32bitreg(SYS_STATUS_ID,
            (SYS_STATUS_ALL_TX | SYS_STATUS_ALL_RX_ERR | SYS_STATUS_ALL_RX_TO |
...
void dwt_setrxtimeout(uint16 time)
{
    ...
    temp = dwt_read8bitoffsetreg(SYS_CFG_ID,
            3); // Read at offset 3 to get the upper byte only
...
void dwt_readeventcounters(dwt_deviceentcnts_t *counters)
{
    ...
    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID,
            EVC_PHE_OFFSET); // Read sync loss (31-16), PHE (15-0)
    ...
    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID,
            EVC_STO_OFFSET); // Read PTO (31-16), SFDTO (15-0)
    counters->PTO = (temp >> 16) & 0xFFF;
    counters->SFDTO = temp & 0xFFF;

    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID,
            EVC_FWTO_OFFSET); // Read RX TO (31-16), TXFRAME (15-0)
    counters->TXF = (temp >> 16) & 0xFFF;
    counters->RTO = temp & 0xFFF;

    temp = dwt_read32bitoffsetreg(DIG_DIAG_ID,
            EVC_HPW_OFFSET); // Read half period warning events
    counters->HPW = temp & 0xFFF;
    counters->TXW = (temp >> 16) &
                    0xFFF;                       // Power-up warning events
...
uint32 _dwt_computetxpowersetting(uint32 ref_powerreg, int32 power_adj)
{
    ...
    for(i = 0; i < 4; i++) {
        ...
        new_regval |=
                ((uint32) ((new_da_attn << 5) | new_mixer_gain)) << (i * 8);
    }
